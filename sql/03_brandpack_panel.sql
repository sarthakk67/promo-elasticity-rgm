...
CREATE OR REPLACE TABLE brandpack AS
WITH sku_week AS (
    SELECT manufacturer, sub_commodity_desc, brand, product_id, week_no,
           SUM(units)                                  AS units,
           SUM(sales_value)                            AS sales_value,
           SUM(retail_disc)                            AS retail_disc,
           SUM(coupon_match_disc)                      AS coupon_match_disc,
           SUM(sales_value) / SUM(units)               AS paid_price,
           (SUM(sales_value) + SUM(retail_disc) + SUM(coupon_match_disc)) / SUM(units) AS shelf_price,
           -- promo exposure, unit-weighted so a promoted SKU counts by its size
           SUM(units * on_display) / SUM(units)        AS display_share,
           SUM(units * on_mailer)  / SUM(units)        AS mailer_share
    FROM panel_all
    WHERE commodity_desc = 'SOFT DRINKS'
    GROUP BY 1, 2, 3, 4, 5
),
base_w AS (   -- fixed base weight: SKU share of its brand-pack over ALL weeks
    SELECT product_id, manufacturer, sub_commodity_desc,
           SUM(units) AS sku_units,
           SUM(units) / SUM(SUM(units)) OVER (PARTITION BY manufacturer, sub_commodity_desc) AS w
    FROM sku_week GROUP BY 1, 2, 3
)
SELECT
    s.manufacturer,
    ...
    -- fixed-weight geometric price index (weights renormalised over SKUs present)
    EXP(SUM(b.w * LN(s.paid_price)) / SUM(b.w))                 AS price_index,
    EXP(SUM(b.w * LN(s.shelf_price)) / SUM(b.w))                AS shelf_index,
    -- naive unit value, kept ONLY so the mix-bias gap can be quantified
    SUM(s.sales_value) / SUM(s.units)                           AS unit_value,
    SUM(s.units * s.display_share) / SUM(s.units)               AS display_share,
    SUM(s.units * s.mailer_share)  / SUM(s.units)               AS mailer_share
FROM sku_week s
JOIN base_w b USING (product_id)
WHERE s.paid_price > 0 AND s.shelf_price > 0
GROUP BY s.manufacturer, s.sub_commodity_desc, s.week_no
